...
public class BasicLTLModelChecker implements LTLModelChecker {
    ...
    private GNBA computeGNBA(TransitionSystem ts, LTLFormula formula) {
            ...
            for (Guess potentialSuccessor : elementarySets) {
                ...
                for (LTLFormula ltlFormula : closure) {
                    if (ltlFormula instanceof LTLNextFormula nextFormula) {
                        if (one.isAssumed(nextFormula) != potentialSuccessor.isAssumed(nextFormula.getFormula())) {
                            violates = true;
                            break;
                        }
                    } else if (ltlFormula instanceof LTLUntilFormula untilFormula) {
                        boolean lhs = one.isAssumed(untilFormula);
                        boolean rhs = one.isAssumed(untilFormula.getRight()) ||
                                (one.isAssumed(untilFormula.getLeft()) && potentialSuccessor.isAssumed(untilFormula));
                        ...
                        boolean rhs = one.isAssumed(eventuallyFormula.getFormula()) ||
                        ...
                        boolean rhs = one.isAssumed(alwaysFormula.getFormula()) &&
                ...
                gnbaBuilder.addTransition(one.toString(), assumedAtomicPropositions.toString(), potentialSuccessor.toString());
            }
        }
        ...
        for (LTLFormula ltlFormula : closure) {
            if (ltlFormula instanceof LTLUntilFormula untilFormula) {
                ...
                Set<String> acceptingSet = new HashSet<>();
                for (Guess state : elementarySets) {
                    if(!state.isAssumed(eventuallyFormula) || state.isAssumed(eventuallyFormula.getFormula())) {
                        acceptingSet.add(state.toString());
                    }
                }
                gnbaBuilder.addAcceptingSet(acceptingSet);
    ...
    public Set<LTLFormula> reduceClosure(Set<LTLFormula> closure) {
        ...
        for (LTLFormula ltlFormula : closure) {
            if (ltlFormula instanceof LTLTrueFormula || ltlFormula instanceof LTLFalseFormula ||
                    ltlFormula instanceof LTLNotFormula || ltlFormula instanceof LTLParenthesisFormula) {
                continue;
            }
    ...
    private Set<Guess> computeElementarySets(Set<String> atomicPropositions, Set<LTLFormula> closure) {
        ...
        while (true) {
            Optional<LTLFormula> unassigned = reduced.stream().filter(f -> !assignment.assignsFormulaOrNegation(f)).findFirst();
